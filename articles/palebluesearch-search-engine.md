...
ただ、既存の検索 API を使っていて一つ気になることがあった。**検索結果がいつの情報なのかわからない**。Google の検索結果にもタイムスタンプはあるが、API 経由で取れるものは限定的だ。AI エージェントが「2024年の最新情報」を求めているのに、3年前の記事を根拠にしていたら意味がない。

そこで自分で検索エンジンを作ることにした。すべての検索結果に `indexed_at`（クロール時刻）と `published_at`（公開日）を返す、AI エージェント向けの検索 API。名前は **PaleBlueSearch**。
...
| Crawler | 分散クローラー | aiohttp + BeautifulSoup |
...
## freshness metadata: この検索エンジンの存在理由
...
これが PaleBlueSearch の差別化ポイント。Google や Bing の API にはこのメタデータが体系的に含まれていない。
...
4. **AI エージェントに必要なのは「メタデータ付きの検索結果」**。URL とスニペットだけでなく、鮮度情報があると判断精度が上がる。
